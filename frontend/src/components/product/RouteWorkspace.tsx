...
import { ArrowRight, Clock, Info, LifeBuoy, MapPin, RefreshCw, ShieldAlert } from "lucide-react";
...
export function RouteWorkspace() {
  ...
  const [originLabel, setOriginLabel] = useState("Bandra Reclamation / Hill Rd");
  ...
  const [contexts, setContexts] = useState<Record<string, RouteContextResponse>>({});
  ...
  const fetchRoutes = useCallback(async () => {
    ...
    try {
      ...
      setError(err instanceof Error ? err.message : "Could not plan route with current server.");
    } finally {
      setIsLoading(false);
    }
  ...
  const activeRoute: RouteResponse | undefined =
    comparison?.routes.find((r) => r.id === selectedRouteId) || comparison?.routes[0];
  const activeContext: RouteContextResponse | undefined = activeRoute
    ? contexts[activeRoute.id]
    : undefined;

  // Selected segment object
  const activeSegment = activeRoute?.segments.find((s) => s.id === selectedSegmentId);
  const activeSegmentContext = activeContext?.segments.find((s) => s.segment_id === selectedSegmentId);
  ...
  const handleStartTrip = async () => {
    ...
    try {
      ...
      const arrivalDate = new Date(departureDate.getTime() + (activeRoute.duration_seconds || 1200) * 1000);
      ...
      setTripError(err instanceof Error ? err.message : "Could not start trip.");
    } finally {
  ...
  return (
    <div>
      ...
      <div className="mb-7 flex flex-wrap items-end justify-between gap-4">
        <div>
          ...
          <p className="mt-1 text-xs text-[#62706a]">
            Deterministic contextual evidence across time. Never a numerical score or safety guarantee.
          </p>
        </div>
        <button
      ...
      {/* ── Error Banner ── */}
      {error && (
        <div role="alert" className="mb-6 border-l-4 border-[#b6433d] bg-[#fde8e7] p-4 text-sm text-[#b6433d]">
          <div className="flex items-start gap-2">
            <ShieldAlert className="mt-0.5 h-4 w-4 shrink-0" />
            <div>
              <p className="font-semibold">Route evaluation could not be completed</p>
      ...
      <section className="grid gap-6 lg:grid-cols-[.9fr_1.1fr]">
        ...
        <div className="border border-[#d8ddd7] bg-[#fffefb] p-5 shadow-sm">
          <div>
            <label htmlFor={originInputId} className="block text-xs font-semibold uppercase tracking-wider text-[#62706a]">
              From (Origin)
            </label>
            ...
            <p className="mt-1 text-[11px] text-[#62706a]">
              Pilot area coordinates: {origin.latitude.toFixed(4)}°N, {origin.longitude.toFixed(4)}°E
            </p>
          </div>

          <div className="mt-5">
            <label htmlFor={destInputId} className="block text-xs font-semibold uppercase tracking-wider text-[#62706a]">
              To (Destination)
            </label>
            <div className="mt-1.5 flex items-center border-b border-[#d8ddd7] py-2 text-sm">
            ...
            <p className="mt-1 text-[11px] text-[#62706a]">
              Pilot area coordinates: {destination.latitude.toFixed(4)}°N, {destination.longitude.toFixed(4)}°E
            </p>
          </div>
          ...
          <div className="mt-6 border-t border-[#d8ddd7] pt-5">
            ...
            <p className="mt-3 text-xs leading-5 text-[#62706a]">
              Context conditions change deterministically with the time lens. Pedestrian activity, daylight, and facility operating status update accordingly.
            </p>
          </div>
          ...
          <div className="mt-6 border-t border-[#d8ddd7] pt-5">
            ...
            >
              {isStartingTrip ? "Starting active journey..." : "Start journey with this route"}
              <ArrowRight className="h-4 w-4" />
            </button>
            <p className="mt-2 text-center text-[11px] text-[#62706a]">
              Active-trip location sharing is off by default and requires separate consent.
            </p>
          </div>
        </div>
        ...
        <div>
          ...
          {selectedHelpPointRef && selectedHelpPoint && (
            <div className="mt-2 flex items-center justify-between border border-[#d8ddd7] bg-[#fffefb] p-3 text-xs text-[#53615a]">
              <div>
                <span className="font-semibold text-[#14231d]">Selected Help Point:</span>{" "}
                {selectedHelpPoint.category.replace(/_/g, " ")} ({selectedHelpPoint.verification_status})
                {selectedHelpPoint.contact && ` · Tel: ${selectedHelpPoint.contact}`}
              </div>
              <button
                ...
                return (
                  ...
                  >
                    <div className="flex items-center justify-between">
                      ...
                      <span className="text-xs text-[#62706a]">{minutes} min</span>
                    </div>
                    <div className="mt-1 text-xs text-[#53615a]">{km} km · Walking</div>
                    <div className="mt-2">
                      <span
        ...
        <section className="mt-6 grid gap-6 lg:grid-cols-3">
          <article className="border-t-2 border-[#16756c] bg-[#fffefb] p-6 shadow-sm lg:col-span-2">
            ...
            <h2 className="mt-2 font-serif text-xl font-semibold text-[#14231d]">
              {formatContextBand(activeContext?.route_context_band).label} — {Math.round(activeRoute.duration_seconds / 60)} min walk
            </h2>

            <p className="mt-2 text-sm leading-6 text-[#53615a]">
              {typeof activeContext?.explanation === "object" && activeContext.explanation !== null
                ? JSON.stringify(activeContext.explanation).replace(/[{}"]/g, " ")
            ...
            <div className="mt-6 border-t border-[#d8ddd7] pt-5">
              <h3 className="text-xs font-semibold uppercase tracking-wider text-[#62706a]">
                Route Segments ({activeRoute.segments.length}) — Click to inspect
              </h3>
              <div className="mt-3 flex flex-wrap gap-2">
                {activeRoute.segments.map((seg, sIdx) => {
                  const segCtx = activeContext?.segments.find((s) => s.segment_id === seg.id);
                  ...
                  return (
                    <button
                      key={seg.id}
                      onClick={() => setSelectedSegmentId(isSegSelected ? null : seg.id)}
              ...
              {activeSegment && activeSegmentContext && (
                <div className="mt-4 border border-[#d8ddd7] bg-[#f7f6f1] p-4 text-xs">
                  <div className="flex items-center justify-between font-semibold text-[#14231d]">
                    <span>Selected Segment Details (Sequence {activeSegment.sequence})</span>
                    <span>Length: {activeSegment.length_meters}m · Walk: {Math.round(activeSegment.travel_seconds / 60)} min</span>
                  </div>
                  <div className="mt-2 grid grid-cols-2 gap-3 text-[#53615a]">
                    <div>
                      <span className="font-semibold text-[#14231d]">Strongest Support:</span>{" "}
                      {activeSegmentContext.strongest_support.length > 0
                        ? activeSegmentContext.strongest_support.join(", ")
                        : "Baseline mapped conditions"}
                    </div>
                    <div>
                      <span className="font-semibold text-[#14231d]">Caution Factors:</span>{" "}
                      {activeSegmentContext.caution.length > 0
                        ? activeSegmentContext.caution.join(", ")
                        : "None reported"}
                    </div>
                    <div>
                      <span className="font-semibold text-[#14231d]">Confidence:</span>{" "}
                      {activeSegmentContext.confidence}
                    </div>
                    <div>
                      <span className="font-semibold text-[#14231d]">Coverage:</span>{" "}
            ...
            <dl className="mt-6 grid grid-cols-2 gap-4 border-t border-[#d8ddd7] pt-5 text-sm">
              <div>
                <dt className="text-xs font-semibold uppercase tracking-wider text-[#62706a]">What supports it</dt>
                <dd className="mt-1 font-medium text-[#14231d]">
                  {activeContext?.segments[0]?.strongest_support.join(", ") || "Active commercial corridor, street lighting"}
                </dd>
              </div>
              <div>
                <dt className="text-xs font-semibold uppercase tracking-wider text-[#62706a]">What is uncertain</dt>
                <dd className="mt-1 font-medium text-[#14231d]">
                  {activeContext?.segments[0]?.caution.join(", ") || "Conditions may vary after midnight"}
                </dd>
              </div>
            </dl>
          </article>
          ...
          <aside className="border border-[#d8ddd7] bg-[#fffefb] p-6 shadow-sm">
            ...
            <p className="mt-2 text-xs leading-5 text-[#62706a]">
              Police stations, hospitals, and pharmacies near your route corridor.
            </p>

            <div className="mt-4 space-y-3">
              {helpPoints.slice(0, 3).map((hp) => (
                <div key={hp.reference} className="border-b border-[#d8ddd7] pb-2 text-xs">
      ...
      <div className="mt-6 flex items-start gap-2 border border-[#d8ddd7] bg-[#fffefb] p-4 text-xs text-[#53615a]">
        ...
        <p>
          SaferPath provides contextual awareness derived from public infrastructure and reports. It does not predict or guarantee safety. You make the final travel decisions.
        </p>
      </div>
    </div>
  );
