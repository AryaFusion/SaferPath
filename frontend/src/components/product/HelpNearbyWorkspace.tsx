...
import { Building2, Info, LifeBuoy, Phone, Pill, RefreshCw, Shield, Stethoscope, Train } from "lucide-react";
...
export function HelpNearbyWorkspace() {
  ...
  // Pilot coordinate center: Bandra/Shivaji Park corridor
  const latitude = 19.054;
  const longitude = 72.835;
  ...
  const loadPoints = async () => {
    ...
    try {
      ...
      setError(err instanceof Error ? err.message : "Could not load nearby help points.");
    } finally {
      setIsLoading(false);
    }
  };
  ...
  return (
    <div>
      ...
      <div className="mb-7 flex flex-wrap items-end justify-between gap-4">
        <div>
          ...
          <p className="mt-1 text-xs text-[#62706a]">
            Nearby public assistance facilities with verified operating hours and source telemetry.
          </p>
        </div>
        <button
          onClick={() => void loadPoints()}
          disabled={isLoading}
          className="flex items-center gap-2 border border-[#d8ddd7] bg-[#fffefb] px-4 py-2 text-xs font-semibold text-[#53615a] transition hover:bg-[#f0f2ed]"
        >
      ...
      <div className="mb-6 border-l-4 border-[#b6433d] bg-[#fde8e7] p-5 shadow-sm">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          <div className="flex items-start gap-3">
            ...
            <div>
              <p className="font-semibold text-[#b6433d]">Immediate Emergency Response</p>
              <p className="mt-0.5 text-xs text-[#14231d]">
                If you or someone nearby is in immediate danger, call official emergency services directly. SaferPath does not dispatch emergency services.
              </p>
            </div>
          </div>
          <a
      ...
      {/* ── Help Points List ── */}
      {error && (
        <div role="alert" className="mb-6 border border-[#b6433d] bg-[#fde8e7] p-4 text-xs text-[#b6433d]">
          {error}
        </div>
      )}
      ...
      {isLoading ? (
            ...
            return (
              ...
              >
                <div className="flex items-start justify-between gap-2">
                  <div className="flex items-center gap-2.5">
                    ...
                    <div>
                      ...
                      <span className="text-[10px] text-[#62706a]">Ref: {point.reference.slice(0, 10)}</span>
                    </div>
                  </div>
                ...
                <div className="mt-4 space-y-1.5 text-xs text-[#53615a]">
                  <p>
                    <span className="font-semibold text-[#14231d]">Status:</span>{" "}
                    {point.verification_status}
                  </p>
                  {point.accessibility && (
                    <p>
                      <span className="font-semibold text-[#14231d]">Accessibility:</span>{" "}
                      {point.accessibility}
                    </p>
                  )}
        ...
        <div className="border border-[#d8ddd7] bg-[#fffefb] p-8 text-center text-sm text-[#53615a]">
          ...
          <p className="mt-3 font-semibold text-[#14231d]">No help points matching current filter</p>
          <p className="mt-1 text-xs text-[#62706a]">
      ...
      <div className="mt-8 flex items-start gap-2 border border-[#d8ddd7] bg-[#fffefb] p-4 text-xs text-[#53615a]">
        ...
        <p>
          Information is sourced from official registers and verified partner databases. Unverified raw crowd data is clearly distinguished from SaferPath-verified partners.
        </p>
      </div>
    </div>
  );
