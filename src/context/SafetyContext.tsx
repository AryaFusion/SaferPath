...
interface SafetyContextType {
  ...
  isUsingFixtureHelpPoints: boolean;
}

export const SafetyContext = createContext<SafetyContextType | undefined>(
  undefined,
);
...
export const SafetyProvider: React.FC<{ children: React.ReactNode }> = ({
  ...
  useEffect(() => {
    ...
    if (
      typeof window !== "undefined" &&
      window.location.pathname !== initialNormalized
    ) {
  ...
  const [selectedRouteId, setSelectedRouteId] =
    useState<string>("valhalla-route-1");
  ...
  const [isUsingFixtureHelpPoints, setIsUsingFixtureHelpPoints] =
  ...
  const DEFAULT_SAVED_PLACES: SavedPlace[] = [
    ...
    },
    {
      ...
      name: "Transit Hub",
      label: "Work",
      address: "Dadar Station West, Mumbai",
      coordinates: [72.8433, 19.018],
      isDemo: true,
    },
    {
      id: "sp-3",
    ...
    if (typeof window !== "undefined") {
      ...
      if (saved) {
        try {
          ...
          console.warn(
            "Failed to parse trusted contacts from localStorage:",
            err,
          );
        }
      }
    ...
    if (typeof window !== "undefined") {
      ...
      if (saved) {
        try {
          ...
          console.warn(
            "Failed to parse privacy settings from localStorage:",
            err,
          );
        }
      }
      ...
      try {
        ...
        const legacyRoutes: RouteOption[] = result.routes.map((nr, idx) => ({
          ...
          freshness:
            nr.freshness ||
            (idx === 0
              ? "Observed 10 mins ago"
              : idx === 1
                ? "Observed 25 mins ago"
                : "Observed 1 hour ago"),
          lightingEvidence:
            nr.lightingEvidence ||
            (idx === 0
  ...
  // Load help points and reports
  useEffect(() => {
    getNearbyHelpPoints(19.019, 72.843, 2000)
      .then((apiItems: HelpPointApiItem[]) => {
        const CATEGORY_MAP: Record<string, HelpPoint["category"]> = {
          PHARMACY: "Pharmacy",
        ...
        const STATUS_MAP: Record<string, HelpPoint["status"]> = {
          OPEN: "Open & Lit",
          STAFFED_24H: "24/7 Staffed",
          CLOSED: "Closed currently",
        };
        const mappedPoints: HelpPoint[] = apiItems.map((item, idx) => ({
          id: item.reference || `hp-api-${idx}`,
          name:
            item.contact?.name || CATEGORY_MAP[item.category] || item.category,
          category: CATEGORY_MAP[item.category] || "Pharmacy",
          distanceMeters: 0,
          status: STATUS_MAP[item.operating_status] || "Open & Lit",
          verificationAuthority: item.verification_status || "Backend verified",
          verifiedTime: "Recent",
          address: item.contact?.address || "—",
          phone: item.contact?.phone || item.contact?.telephone,
        }));
        setHelpPoints(mappedPoints);
        setIsUsingFixtureHelpPoints(false);
      })
      .catch((err) => {
        console.warn("Real help points API failed:", err);
        if (import.meta.env.DEV === true) {
          FixtureApiClient.getHelpPoints().then(setHelpPoints);
          setIsUsingFixtureHelpPoints(true);
        }
      });
  ...
  const startTrip = useCallback(
    (routeId?: string, contactId?: string) => {
      ...
      const targetRoute =
        routes.find((r) => r.id === targetId) || selectedRoute || routes[0];
      ...
      const newTrip: ActiveTrip = {
        ...
        routeName: targetRoute.via
          ? `${targetRoute.name} (${targetRoute.via})`
          : targetRoute.name,
  ...
  const logTripCheckIn = useCallback(
    (
      ...
      setActiveTrip((prev) => {
        if (!prev) return null;
        const isEnded =
          eventType === "destination_reached" || eventType === "trip_cancelled";
        return {
          ...prev,
  ...
  const endTrip = useCallback(() => {
    logTripCheckIn(
      "Destination reached. Check-in session completed.",
      "destination_reached",
    );
  }, [logTripCheckIn]);
  ...
  const editSavedPlace = useCallback(
    (id: string, updated: Partial<SavedPlace>) => {
      setSavedPlaces((prev) =>
  ...
  const addPhysicalReport = (
    rep: Omit<PhysicalReport, "id" | "submittedAt">,
  ) => {
  ...
  const editContact = useCallback(
    (id: string, updated: Partial<TrustedContact>) => {
      setContacts((prev) =>
